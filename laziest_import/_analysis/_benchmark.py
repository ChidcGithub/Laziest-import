...
class BenchmarkRunner:
    ...
    def benchmark_lazy_import(
        ...
        def do_lazy_import():
            lz.clear_cache()
            mod = getattr(lz, module_name.split(".", maxsplit=1)[0])
    ...
    def compare_import_methods(
        ...
        speedup = (
            regular_result.avg_time / lazy_result.avg_time if lazy_result.avg_time > 0 else 1.0
        )
...
def benchmark(
    ...
    *,
    ...
    bench_name = name or str(func_name)
...
def benchmark_imports(
    ...
    return BenchmarkReport(
        results=results,
        comparison={c.module_name: c.speedup_factor for c in comparisons},
        recommendations=recommendations,
    )
