...
def show_conflicts(symbol_filter: Optional[str] = None, max_results: int = 20) -> None:
    """
    ...
    # Filter if requested
    if symbol_filter:
        conflicts = {k: v for k, v in conflicts.items() if symbol_filter.lower() in k.lower()}
    ...
    sorted_conflicts = sorted(conflicts.items(), key=lambda x: -len(x[1].locations))[:max_results]
...
def get_conflicts_summary() -> Dict[str, Any]:
    ...
    return {
        ...
        "modules_with_conflicts": dict(sorted(module_counts.items(), key=lambda x: -x[1])[:10]),
        "top_conflicts": [
            {"symbol": symbol, "modules": [m for m, _ in conflict.locations]}
            for symbol, conflict in sorted(conflicts.items(), key=lambda x: -len(x[1].locations))[
                :10
            ]
        ],
    }
