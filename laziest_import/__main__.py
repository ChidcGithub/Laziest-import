...
Usage:
    ...
    python -m laziest_import fix    [--output FILE] [paths...]
    ...
    for node in ast.walk(tree):
        ...
        if isinstance(node, ast.Attribute):
            ...
            if isinstance(current, ast.Name):
                alias = current.id
        if alias and alias in aliases and alias not in found:
...
def _cmd_fix() -> None:
    ...
    aliases = _resolve_aliases()
    if not aliases:
        print("[laziest-import fix] No aliases found. Is laziest-import initialized?")
        sys.exit(1)

    usage = _scan_path_for_aliases(paths, aliases)
    if not usage:
        print("[laziest-import fix] No alias usage detected")
        sys.exit(1)

    lines = ["# Generated by: laziest-import fix", "# Standard imports for detected aliases", ""]
    for alias in sorted(usage):
    ...
    if output:
        with open(output, "w", encoding="utf-8") as f:
            f.write(result)
        print(f"[laziest-import fix] Generated {output}")
    else:
        print(result)
