...
所有函数均已弃用，调用时将发出 FutureWarning。
建议迁移到新的 LazyImport API:
...
def get_module(alias: str) -> Optional[Any]:
    """已弃用 → lz().module.get(alias) 或 lz().alias[alias]"""
...
def reset_all() -> None:
    """已弃用 → lz().cache.clear() + lz().alias.reload() + lz().symbol.index.reset()"""
    _warn("reset_all", "lz().cache.clear() 等组合操作")
    from ._config import reset_all as _reset_all
    _reset_all()


def get_version(alias: str) -> Optional[str]:
    """已弃用 → lz().version.of(alias)"""
...
def search_class(class_name: str) -> Optional[tuple]:
    """已弃用 → lz().symbol.search(name, symbol_type='class')"""
...
def reset_import_stats() -> None:
    """已弃用 → （无直接等价，待定）"""
    _warn("reset_import_stats", "暂无直接等价的新 API")
...
def search_symbol(
    ...
    """已弃用 → lz().symbol.search(name, symbol_type, signature, max_results)"""
...
def enable_symbol_search(
    ...
    """已弃用 → lz().symbol.config.enable() 或 lz().symbol.config.interactive = ..."""
...
def disable_symbol_search() -> None:
    """已弃用 → lz().symbol.config.enabled = False"""
...
def set_symbol_preference(symbol: str, module: str) -> None:
    """已弃用 → lz().symbol.prefer(symbol, module)"""
...
def get_symbol_preference(symbol: str) -> Optional[str]:
    """已弃用 → lz().symbol.preference(symbol)"""
...
def clear_symbol_preference(symbol: str) -> bool:
    """已弃用 → lz().symbol.clear_preference(symbol)"""
...
def list_symbol_conflicts(symbol: str) -> List[Dict[str, Any]]:
    """已弃用 → lz().symbol.conflicts(symbol)"""
...
def set_module_priority(module: str, priority: int) -> None:
    """已弃用 → lz().symbol.config.module_priority 命名空间方法"""
    _warn("set_module_priority", "lz().symbol 模块配置")
    from ._symbol import set_module_priority as _set
    _set(module, priority)


def get_module_priority(module: str) -> int:
    """已弃用 → lz().symbol.config.module_priority"""
    _warn("get_module_priority", "lz().symbol 模块配置")
...
def enable_auto_symbol_resolution(
    ...
    """已弃用 → lz().symbol.config.auto_resolution = True"""
...
def disable_auto_symbol_resolution() -> None:
    """已弃用 → lz().symbol.config.auto_resolution = False"""
...
def build_symbol_index_incremental() -> bool:
    """已弃用 → lz().symbol.index.incremental()"""
...
def set_cache_config(
    ...
    """已弃用 → lz().cache.config.max_size_mb = 200 等"""
...
def enable_file_cache() -> None:
    """已弃用 → lz().cache.files.enabled = True"""
...
def disable_file_cache() -> None:
    """已弃用 → lz().cache.files.enabled = False"""
...
def clear_file_cache(file_path: Optional[str] = None) -> int:
    """已弃用 → lz().cache.files.clear(file_path)"""
...
def enable_background_build(enabled: bool = True) -> None:
    """已弃用 → lz().background.enable(enabled)"""
...
def enable_incremental_index(enabled: bool = True) -> None:
    """已弃用 → lz().background 相关"""
    _warn("enable_incremental_index", "lz().background 相关")
...
def get_incremental_config() -> Dict[str, Any]:
    """已弃用 → lz().background 相关"""
    _warn("get_incremental_config", "lz().background 相关")
...
def get_package_version(package_name: str) -> Optional[str]:
    """已弃用 → lz().version.of(package_name)"""
...
def export_aliases(path: Optional[str] = None, include_categories: bool = True) -> str:
    """已弃用 → lz().alias.export(path, include_categories)"""
...
def register_alias(alias: str, module_name: str) -> None:
    """已弃用 → lz().alias.register(alias, module_name)"""
...
def register_aliases(aliases: Dict[str, str]) -> List[str]:
    """已弃用 → lz().alias.register_many(aliases)"""
...
def unregister_alias(alias: str) -> bool:
    """已弃用 → lz().alias.unregister(alias) 或 del lz().alias[alias]"""
...
def install_package(
    ...
    """已弃用 → lz().install.package(package_name, ...)"""
...
def enable_auto_install(
    ...
    """已弃用 → lz().install.enable(interactive=...)"""
...
def set_pip_index(url: Optional[str]) -> None:
    """已弃用 → lz().install 相关配置"""
    _warn("set_pip_index", "lz().install 相关配置")
...
async def import_multiple_async(aliases: List[str]) -> Dict[str, Any]:
    """已弃用 → await lz().async.fetch(*aliases)"""
...
def enable_retry(
    ...
    """已弃用 → lz.config.retry.enabled = True"""
...
def disable_retry() -> None:
    """已弃用 → lz.config.retry.enabled = False"""
...
def which(symbol_name: str, module_hint: Optional[str] = None) -> Optional[Any]:
    """已弃用 → lz().symbol.which(symbol_name, module_hint)"""
...
def which_all(symbol_name: str) -> List[Any]:
    """已弃用 → lz().symbol.which_all(symbol_name)"""
...
def analyze_source(source: str, file_path: str = "<string>") -> Any:
    """已弃用 → lz().analyze.code(source, file_path)"""
...
def analyze_directory(
    ...
    """已弃用 → lz().analyze.dir(dir_path, recursive, exclude)"""
...
def detect_environment() -> Any:
    """已弃用 → lz().analyze 相关"""
    _warn("detect_environment", "lz().analyze 相关")
...
def show_environment() -> None:
    """已弃用 → lz().analyze 相关"""
    _warn("show_environment", "lz().analyze 相关")
...
def save_preferences() -> bool:
    """已弃用 → 偏好管理"""
    _warn("save_preferences", "偏好管理 API")
...
def load_preferences() -> Dict[str, str]:
    """已弃用 → 偏好管理"""
    _warn("load_preferences", "偏好管理 API")
...
def apply_preferences() -> None:
    """已弃用 → 偏好管理"""
    _warn("apply_preferences", "偏好管理 API")
...
def clear_preferences() -> bool:
    """已弃用 → 偏好管理"""
    _warn("clear_preferences", "偏好管理 API")
...
def get_preferences_path() -> Path:
    """已弃用 → 偏好管理"""
    _warn("get_preferences_path", "偏好管理 API")
...
def benchmark_imports(aliases: List[str], iterations: int = 10) -> Any:
    """已弃用 → lz().profile 相关"""
    _warn("benchmark_imports", "lz().profile 相关")
