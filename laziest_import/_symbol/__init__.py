...
Internal exports (backward compatibility):
    ...
    - _SYMBOL_INDEX_LOCK
    - _SYMBOL_CACHE, _STDLIB_SYMBOL_CACHE, _THIRD_PARTY_SYMBOL_CACHE
"""
...
def _infer_context() -> Set[str]:
    ...
    from .._config import _LAZY_MODULES, _MODULE_PRIORITY as _fuzzy_MODULE_PRIORITY, _ALIAS_MAP as _fuzzy_ALIAS_MAP
    ...
    for mod_name in sys.modules.keys():
        if mod_name and not mod_name.startswith("_"):
            ...
            if base_module in _fuzzy_MODULE_PRIORITY or base_module in _fuzzy_ALIAS_MAP.values():
                loaded.add(base_module)
...
def _build_incremental_symbol_index(timeout: float = 30.0) -> bool:
    ...
    if total_changes > len(_TRACKED_PACKAGES) * 0.5:
        if _DEBUG_MODE:
...
def _remove_package_symbols(package_name: str) -> None:
    ...
    for cache in (_SYMBOL_CACHE, _STDLIB_SYMBOL_CACHE, _THIRD_PARTY_SYMBOL_CACHE):
        to_remove = []
